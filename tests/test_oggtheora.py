...
class TOggTheora(TestCase, TOggFileTypeMixin):
    ...
    def setUp(self):
        self.filename = get_temp_copy(os.path.join(DATA_DIR, 'sample.oggtheora'))

        self.audio = OggTheora(self.filename)
        self.audio2 = OggTheora(os.path.join(DATA_DIR, 'sample_length.oggtheora'))
        self.audio3 = OggTheora(os.path.join(DATA_DIR, 'sample_bitrate.oggtheora'))
    ...
    def test_vendor(self):
        self.failUnless(self.audio.tags.vendor.startswith('Xiph.Org libTheora'))
