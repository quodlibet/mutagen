...
class TOggPage(TestCase):
    ...
    def test_flags(self):
        self.assertTrue(self.page.first)
        self.assertFalse(self.page.continued)
        self.assertFalse(self.page.last)
        self.assertTrue(self.page.complete)
        ...
        for first in [True, False]:
            ...
            for last in [True, False]:
                ...
                for continued in [True, False]:
                    ...
                    self.assertEqual(self.page.first, first)
                    self.assertEqual(self.page.last, last)
                    self.assertEqual(self.page.continued, continued)

    def test_flags_next_page(self):
        page = OggPage(self.fileobj)
        self.assertFalse(page.first)
        self.assertFalse(page.continued)
        self.assertFalse(page.last)

    def test_length(self):
        # Always true for Ogg Vorbis files
        self.assertEqual(self.page.size, 58)
        self.assertEqual(len(self.page.write()), 58)

    def test_first_metadata_page_is_separate(self):
        self.assertFalse(OggPage(self.fileobj).continued)
    ...
    def test_crappy_fragmentation(self):
        ...
        self.assertEqual(OggPage.to_packets(pages), packets)
    ...
    def test_replace_continued(self):
        ...
        self.assertEqual(written, [b"quuuxbar"])
    ...
    def test_renumber(self):
        ...
        self.assertEqual([page.sequence for page in pages], [10, 11, 12])

        fileobj.seek(0)
        OggPage.renumber(fileobj, 1, 20)
        fileobj.seek(0)
        pages = [OggPage(fileobj) for i in range(3)]
        self.assertEqual([page.sequence for page in pages], [20, 21, 22])
    ...
    def test_renumber_muxed(self):
        ...
        self.assertEqual(pages[1].serial, 2)
        self.assertEqual(pages[1].sequence, 100)
        pages.pop(1)
        self.assertEqual(
    ...
    def test_to_packets_mixed_stream(self):
        ...
        self.assertRaises(ValueError, OggPage.to_packets, self.pages)
    ...
    def test_page_max_size_alone_too_big(self):
        ...
        page.complete = True
        self.assertRaises(ValueError, page.write)
    ...
    def test_complete_zero_length(self):
        ...
        self.assertEqual(OggPage.to_packets([new_page]), packets)

    def test_too_many_packets(self):
        packets = [b"1"] * 3000
        pages = OggPage.from_packets(packets)
        map(OggPage.write, pages)
        self.assertTrue(len(pages) > 3000 // 255)
    ...
    def test_read_max_size(self):
        ...
        self.assertEqual(OggPage(fileobj), page)
        self.assertEqual(OggPage(fileobj), page2)
        self.assertRaises(EOFError, OggPage, fileobj)

    def test_invalid_version(self):
        page = OggPage()
        OggPage(BytesIO(page.write()))
        page.version = 1
        self.assertRaises(OggError, OggPage, BytesIO(page.write()))

    def test_not_enough_lacing(self):
        data = OggPage().write()[:-1] + b"\x10"
        self.assertRaises(OggError, OggPage, BytesIO(data))
    ...
    def test_not_equal(self):
        self.assertNotEqual(OggPage(), 12)
    ...
    def test_find_last_no_serial(self):
        ...
        for i, page in enumerate(pages):
            page.sequence = i
        data = BytesIO(b"".join([page.write() for page in pages]))
        self.assertTrue(OggPage.find_last(data, pages[0].serial + 1) is None)

    def test_find_last_invalid(self):
        data = BytesIO(b"if you think this is an Ogg, you're crazy")
        self.assertRaises(OggError, OggPage.find_last, data, 0)
    ...
    def test_crc_py25(self):
        ...
        self.assertEqual(uint_data, int_data)

    def tearDown(self):
        self.fileobj.close()


class TOggFileTypeMixin:
    ...
    def test_no_tags(self):
        self.assertFalse(self.audio.tags)
        self.assertFalse(self.audio.tags is None)

    def test_vendor_safe(self):
        self.audio["vendor"] = "a vendor"
        self.audio.save()
        audio = self.Kind(self.filename)
        self.assertEqual(audio["vendor"], ["a vendor"])
    ...
    def test_set_two_tags(self):
        ...
        self.audio.save()
        audio = self.Kind(self.filename)
        self.assertEqual(len(audio.tags.keys()), 2)
        self.assertEqual(audio["foo"], ["a"])
        self.assertEqual(audio["bar"], ["b"])
        self.scan_file()

    def test_save_twice(self):
        self.audio.save()
        self.audio.save()
        self.assertEqual(self.Kind(self.filename).tags, self.audio.tags)
        self.scan_file()
    ...
    def test_delete(self):
        ...
        self.audio.save()
        audio = self.Kind(self.filename)
        self.assertEqual(self.audio["foobar"], audio["foobar"])
    ...
    def test_really_big(self):
        ...
        self.audio.save()
        audio = self.Kind(self.filename)
        self.assertEqual(audio["foo"], ["foo" * 2 ** 16])
        self.assertEqual(audio["bar"], ["bar" * 2 ** 16])
        self.assertEqual(audio["baz"], ["quux" * 2 ** 16])
        self.scan_file()
    ...
    def test_invalid_open(self):
        self.assertRaises(OggError, self.Kind,
                              os.path.join(DATA_DIR, 'xing.mp3'))

    def test_invalid_delete(self):
        self.assertRaises(OggError, self.audio.delete,
                              os.path.join(DATA_DIR, 'xing.mp3'))

    def test_invalid_save(self):
        self.assertRaises(OggError, self.audio.save,
                              os.path.join(DATA_DIR, 'xing.mp3'))
    ...
    def ogg_reference(self, filename):
        ...
        if have_ogginfo:
            self.assertEqual(call_ogginfo(filename), 0,
                             msg=f"ogginfo failed on {filename}")
        ...
        if have_oggz_validate:
            ...
            self.assertEqual(call_oggz_validate(filename), 0,
                             msg=f"oggz-validate failed on {filename}")
    ...
    def test_mime_secondary(self):
        self.assertTrue('application/ogg' in self.audio.mime)
