...
class TMid3cp(_TTools):
    ...
    def test_merge(self):
        ...
        id3.delete()
        id3.add(mutagen.id3.TALB(text=["foo"]))
        id3.save(v2_version=3)

        target = ID3()
        target.add(mutagen.id3.TPE1(text=["bar", "quux"]))
        ...
        assert result.getall("TALB")[0].text == ["foo"]
        assert result.getall("TPE1")[0].text == ["bar", "quux"]
    ...
    def test_copy(self):
        ...
        self.assertEqual(original_id3, copied_id3)

        for key in original_id3:
            # Go through every tag in the original file, and check that it's
            # present and correct in the copy
            self.assertTrue(key in copied_id3)
            self.assertEqual(copied_id3[key], original_id3[key])
    ...
    def test_exclude_single_tag(self):
        ...
        self.assertTrue('TLEN' in original_id3)
        self.assertFalse('TLEN' in copied_id3)
    ...
    def test_exclude_multiple_tag(self):
        ...
        self.assertTrue('TLEN' in original_id3)
        self.assertTrue('TCON' in original_id3)
        self.assertTrue('TALB' in original_id3)
        self.assertFalse('TLEN' in copied_id3)
        self.assertFalse('TCON' in copied_id3)
        self.assertFalse('TALB' in copied_id3)
    ...
    def test_no_src_header(self):
        ...
        try:
            ...
            self.assertTrue("No ID3 header found" in err)
        finally:
            os.unlink(blank_file2)

    def test_verbose(self):
        err = self.call2(self.filename, "--verbose", self.blank_file)[2]
        self.assertTrue('mp3 contains:' in err)
        self.assertTrue('Successfully saved' in err)
