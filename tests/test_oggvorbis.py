...
class TOggVorbis(TestCase, TOggFileTypeMixin):
    ...
    def test_module_delete(self):
        ...
        self.scan_file()
        self.assertFalse(OggVorbis(self.filename).tags)

    def test_bitrate(self):
        self.assertEqual(112000, self.audio.info.bitrate)
    ...
    def test_invalid_not_first(self):
        with open(self.filename, "rb") as h:
            page = OggPage(h)
        page.first = False
        self.assertRaises(error, OggVorbisInfo, BytesIO(page.write()))
    ...
    def test_avg_bitrate(self):
        ...
        self.assertEqual(info.bitrate, 32768)
    ...
    def test_overestimated_bitrate(self):
        ...
        page.packets[0] = packet
        info = OggVorbisInfo(BytesIO(page.write()))
        self.assertEqual(info.bitrate, 65536)
    ...
    def test_vorbiscomment(self):
        ...
        if ogg is None:
            return
        self.assertTrue(ogg.vorbis.VorbisFile(self.filename))
    ...
    def test_vorbiscomment_big(self):
        ...
        self.assertEqual(self.audio["foo"], vfc["foo"])
    ...
    def test_vorbiscomment_delete(self):
        ...
        if ogg is None:
            return
        vfc = ogg.vorbis.VorbisFile(self.filename).comment()
        self.assertEqual(vfc.keys(), ["VENDOR"])
    ...
    def test_vorbiscomment_delete_readd(self):
        ...
        if ogg is None:
            return
        vfc = ogg.vorbis.VorbisFile(self.filename).comment()
        self.assertEqual(self.audio["foobar"], vfc["foobar"])
        self.assertTrue("FOOBAR" in vfc.keys())
        self.assertTrue("VENDOR" in vfc.keys())

    def test_huge_tag(self):
        vorbis = self.Kind(
            os.path.join(DATA_DIR, "multipagecomment.ogg"))
        self.assertTrue("big" in vorbis.tags)
        self.assertTrue("bigger" in vorbis.tags)
        self.assertEqual(vorbis.tags["big"], ["foobar" * 10000])
        self.assertEqual(vorbis.tags["bigger"], ["quuxbaz" * 10000])
        self.scan_file()
    ...
    def test_save_split_setup_packet(self):
        ...
        self.assertEqual(self.audio.tags, tags)
    ...
    def test_save_split_setup_packet_reference(self):
        ...
        for key in self.audio:
            self.assertEqual(vfc[key], self.audio[key])
    ...
    def test_mime(self):
        self.assertTrue("audio/vorbis" in self.audio.mime)
