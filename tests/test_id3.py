...
class TID3Read(TestCase):
    ...
    def test_PIC_in_23(self):
        ...
        try:
            with open(filename, 'wb') as h:
                ...
                h.write(
                    b'ID3\x03\x00\x00\x00\x00\x08\x00PIC\x00\x00\x00'
    ...
    def test_bad_extended_header_flags(self):
        ...
        self.failUnlessEqual(id3['TIT2'].text, ['Punk To Funk'])
...
class TID3Header(TestCase):
    ...
    def test_header_2_4_extended(self):
        fileobj = BytesIO(b'ID3\x04\x00\x40\x00\x00\x00\x00\x00\x00\x00\x05\x5a')
        header = ID3Header(fileobj)
        self.assertEquals(header._extdata, b'\x5a')

    def test_header_2_4_extended_unsynch_size(self):
        fileobj = BytesIO(b'ID3\x04\x00\x40\x00\x00\x00\x00\x00\x00\x00\xff\x5a')
        self.assertRaises(ID3Error, ID3Header, fileobj)

    def test_header_2_4_extended_but_not(self):
        fileobj = BytesIO(b'ID3\x04\x00\x40\x00\x00\x00\x00TIT1\x00\x00\x00\x01a')
        header = ID3Header(fileobj)
    ...
    def test_header_2_3_extended(self):
        fileobj = BytesIO(
            b'ID3\x03\x00\x40\x00\x00\x00\x00\x00\x00\x00\x06\x00\x00\x56\x78\x9a\xbc'
        )
        header = ID3Header(fileobj)
        self.assertEquals(header._extdata, b'\x00\x00\x56\x78\x9a\xbc')

    def test_negative_header_size(self):
        fileobj = BytesIO(b'ID3\x04B@\x00\x00\x00\x00\x00\x00\x00\x00')
        self.failUnlessRaises(ID3Error, ID3Header, fileobj)
...
class TID3Tags(TestCase):
    ...
    def test_getlist(self):
        self.assertTrue(
            self.i.getall('FOOB')
            in [[self.frames[2], self.frames[3]], [self.frames[3], self.frames[2]]]
        )
...
class Issue97_UpgradeUnknown23(TestCase):
    ...
    def test_unknown(self):
        ...
        unknown = ID3(self.filename, known_frames={'TPE1': TPE1}, translate=False)
...
class TID3Write(TestCase):
    ...
    def test_padding_remove_add_padding(self):
        ...
        self.assertEqual(os.path.getsize(self.filename), old_size - old_padding)
        old_size = old_size - old_padding
        tag.save(padding=lambda x: 137)
        self.assertEqual(os.path.getsize(self.filename), old_size + 137)
    ...
    def test_ape_id3_lookalike(self):
        ...
        ape_tag = APEv2()
        ape_tag['oh'] = ['noooooooooo0000000000000000000000000000000000ooooooooooo']
        ape_tag.save(self.filename)
    ...
    def test_replaceframe(self):
        f = ID3(self.filename)
        self.assertEquals(f['TPE1'], ['piman', 'jzig'])
...
class TID3Misc(TestCase):
    ...
    def test_determine_bpi(self):
        ...
        self.assertTrue(determine_bpi(data + b'\x00' * 1000, Frames) is BitPaddedInt)
        ...
        d = get_frame_data(b'TPE2', 1000) + get_frame_data(b'TPE2', 10) + b'\x01' * 875
