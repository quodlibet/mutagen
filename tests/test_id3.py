...
from mutagen.id3._tags import ID3Header, ID3SaveConfig, determine_bpi, save_frame
...
class TID3Read(TestCase):
    ...
    def test_bad_tyer(self):
        ...
        self.assertFalse("TYER" in audio)
        self.assertTrue("TIT2" in audio)
    ...
    def test_handle_tyer_full_date(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDRC"], "2022-02-14")
    ...
    def test_handle_tyer_full_date_with_tdat(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDRC"], "2022-04-05")
    ...
    def test_tags(self):
        tags = ID3(self.v22)
        self.assertTrue(tags["TRCK"].text == ["3/11"])
        self.assertTrue(tags["TPE1"].text == ["Anais Mitchell"])

    def test_load_v1(self):
        tags = ID3(self.silence_v1)
        self.assertEqual(tags["TALB"], "Quod Libet Test Data")
    ...
    def test_load_v1_v2(self):
        ...
        # From ID3v2
        self.assertEqual(tags["TPE1"].text, ["Anais Mitchell"])
        # From ID3v1
        self.assertEqual(tags["TALB"].text, ["Hymns for the Exiled"])
    ...
    def test_load_v1_v2_precedence(self):
        tags = ID3(self.v1v2_combined)
        self.assertEqual(tags["TRCK"].text, ["3/11"])  # i.e. not 123

        # ID3v2 has TYER=2004 (which isn't a valid v2.4 frame),
        # ID3v1 has TDRC=1337.
        self.assertEqual(str(tags["TDRC"].text[0]), "1337")
        with self.assertRaises(KeyError):
            tags["TYER"]
        ...
        self.assertEqual(str(tags["TYER"].text[0]), "2004")
        with self.assertRaises(KeyError):
            tags["TDRC"]
    ...
    def test_load_v1_comment(self):
        ...
        # From ID3v2
        self.assertTrue("Waterbug Records, www.anaismitchell.com" in comments)
        # From ID3v1
        self.assertTrue("v1 comment" in comments)
    ...
    def test_load_v23_unsynch(self):
        ...
        self.assertEqual(id3["TPE1"], ["Nina Simone"])

    def test_v24_extended_header(self):
        id3 = ID3(self.extended_header)
        self.assertEqual(id3["TIT2"], ["One Second of Silence"])
    ...
    def test_bad_extended_header_flags(self):
        ...
        self.assertFalse(id3.f_extended)
        self.assertTrue("TIT2" in id3)
        self.assertTrue("TALB" in id3)
        self.assertEqual(id3["TIT2"].text, ["Punk To Funk"])

    def test_no_known_frames(self):
        id3 = ID3(self.silence, known_frames={})
        self.assertEqual(0, len(id3.keys()))
        self.assertEqual(9, len(id3.unknown_frames))
    ...
    def test_23_multiframe_hack(self):
        ...
        self.assertEqual(8, len(id3.keys()))
        self.assertEqual(0, len(id3.unknown_frames))
        self.assertEqual('Quod Libet Test Data', id3['TALB'])
        self.assertEqual('Silence', str(id3['TCON']))
        self.assertEqual('Silence', str(id3['TIT1']))
        self.assertEqual('Silence', str(id3['TIT2']))
        self.assertEqual(3000, +id3['TLEN'])
        self.assertEqual(['piman', 'jzig'], id3['TPE1'])
        self.assertEqual('02/10', id3['TRCK'])
        self.assertEqual(2, +id3['TRCK'])
        self.assertEqual('2004', id3['TDRC'])
    ...
    def test_chap_subframes(self):
        ...
        self.assertEqual(chap.sub_frames.getall("TDRC")[0], "2006")
        self.assertFalse(chap.sub_frames.getall("TYER"))
        id3.update_to_v23()
        self.assertEqual(chap.sub_frames.getall("TYER")[0], "2006")
    ...
    def test_ctoc_subframes(self):
        ...
        self.assertEqual(ctoc.sub_frames.getall("TDRC")[0], "2006")
        self.assertFalse(ctoc.sub_frames.getall("TYER"))
        id3.update_to_v23()
        self.assertEqual(ctoc.sub_frames.getall("TYER")[0], "2006")
    ...
    def test_pic(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["APIC:cover"].mime, "image/png")
    ...
    def test_tyer(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDRC"], "2006")
    ...
    def test_tyer_tdat(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDRC"], "2006-03-06")
    ...
    def test_tyer_tdat_time(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDRC"], "2006-03-06 11:27:00")
    ...
    def test_tory(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TDOR"], "2006")
    ...
    def test_ipls(self):
        ...
        id3.update_to_v24()
        self.assertEqual(id3["TIPL"], [["a", "b"], ["c", "d"]])
...
class TID3Header(TestCase):
    ...
    def test_header_silence(self):
        ...
        self.assertEqual(header.version, (2, 3, 0))
        self.assertEqual(header.size, 1314)
    ...
    def test_header_2_2(self):
        ...
        header = ID3Header(fileobj)
        self.assertEqual(header.version, (2, 2, 0))
    ...
    def test_header_2_4_extended(self):
        ...
        header = ID3Header(fileobj)
        self.assertEqual(header._extdata, b'\x5a')
    ...
    def test_header_2_4_extended_but_not(self):
        ...
        header = ID3Header(fileobj)
        self.assertEqual(header._extdata, b'')
    ...
    def test_header_2_3_extended(self):
        ...
        header = ID3Header(fileobj)
        self.assertEqual(header._extdata, b'\x00\x00\x56\x78\x9a\xbc')
...
class TID3Tags(TestCase):
    ...
    def test_apic_duplicate_hash(self):
        id3 = ID3Tags()
        for _i in range(10):
            apic = APIC(encoding=0, mime="b", type=3, desc="", data=b"a")
            id3._add(apic, False)
    ...
    def test_text_duplicate_frame_different_encoding(self):
        id3 = ID3Tags()
        frame = TPE2(encoding=Encoding.LATIN1, text=["foo"])
        id3._add(frame, False)
        assert id3.getall("TPE2")[0].encoding == Encoding.LATIN1
        frame = TPE2(encoding=Encoding.LATIN1, text=["bar"])
        id3._add(frame, False)
        assert id3.getall("TPE2")[0].encoding == Encoding.LATIN1
        frame = TPE2(encoding=Encoding.UTF8, text=["baz\u0400"])
        id3._add(frame, False)
    ...
    def test_update_v22_add(self):
        ...
        self.assertEqual(tt1.encoding, tit1.encoding)
        self.assertEqual(tt1.text, tit1.text)
        self.assertTrue('TT1' not in id3)

    def test_getnormal(self):
        self.assertEqual(self.i.getall("BLAH"), [self.frames[0]])
        self.assertEqual(self.i.getall("QUUX"), [self.frames[1]])
        self.assertEqual(self.i.getall("FOOB:ar"), [self.frames[2]])
        self.assertEqual(self.i.getall("FOOB:az"), [self.frames[3]])
    ...
    def test_delnormal(self):
        self.assertTrue("BLAH" in self.i)
        self.i.delall("BLAH")
        self.assertTrue("BLAH" not in self.i)

    def test_delone(self):
        self.i.delall("FOOB:ar")
        self.assertEqual(self.i.getall("FOOB"), [self.frames[3]])

    def test_delall(self):
        self.assertTrue("FOOB:ar" in self.i)
        self.assertTrue("FOOB:az" in self.i)
        self.i.delall("FOOB")
        self.assertTrue("FOOB:ar" not in self.i)
        self.assertTrue("FOOB:az" not in self.i)
    ...
    def test_setone(self):
        ...
        self.assertEqual(self.i["FOOB:ar"], t)
        self.assertEqual(self.i.getall("FOOB"), [t])
    ...
    def test_settwo(self):
        ...
        self.assertEqual(self.i["FOOB:ar"], t)
        self.assertEqual(self.i["FOOB:az"], t2)
        self.assertTrue(self.i.getall("FOOB") in [[t, t2], [t2, t]])
...
class ID3v1Tags(TestCase):
    ...
    def test_album(self):
        self.assertEqual('Quod Libet Test Data', self.id3['TALB'])

    def test_genre(self):
        self.assertEqual('Darkwave', self.id3['TCON'].genres[0])

    def test_title(self):
        self.assertEqual('Silence', str(self.id3['TIT2']))

    def test_artist(self):
        self.assertEqual(['piman'], self.id3['TPE1'])

    def test_track(self):
        self.assertEqual('2', self.id3['TRCK'])
        self.assertEqual(2, +self.id3['TRCK'])

    def test_year(self):
        self.assertEqual('2004', self.id3['TDRC'])

    def test_v1_not_v11(self):
        self.id3["TRCK"] = TRCK(encoding=0, text="32")
        tag = MakeID3v1(self.id3)
        self.assertTrue(32, ParseID3v1(tag)["TRCK"])
        del self.id3["TRCK"]
        tag = MakeID3v1(self.id3)
        tag = tag[:125] + b'  ' + tag[-1:]
        self.assertFalse("TRCK" in ParseID3v1(tag))
    ...
    def test_nulls(self):
        ...
        self.assertEqual(b'abcd'.decode('latin1'), tags['TPE1'])
        self.assertEqual(b'hijklmn'.decode('latin1'), tags['TIT2'])
    ...
    def test_nonascii(self):
        ...
        self.assertEqual(b'qrst\xfcv'.decode('latin1'), tags['TALB'])
        self.assertEqual('wxyz', tags['COMM'])
        self.assertEqual("3", tags['TRCK'])
        self.assertEqual("1234", tags['TDRC'])

    def test_roundtrip(self):
        frames = {}
        for key in ["TIT2", "TALB", "TPE1", "TDRC"]:
            frames[key] = self.id3[key]
        self.assertEqual(ParseID3v1(MakeID3v1(frames)), frames)

    def test_make_from_empty(self):
        empty = b'TAG' + b'\x00' * 124 + b'\xff'
        self.assertEqual(MakeID3v1({}), empty)
        self.assertEqual(MakeID3v1({'TCON': TCON()}), empty)
        self.assertEqual(
    ...
    def test_invalid(self):
        self.assertTrue(ParseID3v1(b"") is None)

    def test_invalid_track(self):
        tag = {}
        tag["TRCK"] = TRCK(encoding=0, text="not a number")
        v1tag = MakeID3v1(tag)
        self.assertFalse("TRCK" in ParseID3v1(v1tag))
...
class TestWriteID3v1(TestCase):
    ...
    def failIfV1(self):
        with open(self.filename, "rb") as fileobj:
            fileobj.seek(-128, 2)
            self.assertFalse(fileobj.read(3) == b"TAG")

    def failUnlessV1(self):
        with open(self.filename, "rb") as fileobj:
            fileobj.seek(-128, 2)
            self.assertTrue(fileobj.read(3) == b"TAG")
...
class Issue97_UpgradeUnknown23(TestCase):
    ...
    def test_unknown(self):
        ...
        self.assertEqual(orig.version, (2, 3, 0))
        ...
        self.assertEqual(unknown.unknown_frames[0][:4], b"TIT2")
        # save as 2.3
        unknown.save(v2_version=3)
        # load again with support for TIT2, all should be there again
        new = ID3(self.filename)
        self.assertEqual(new["TIT2"].text, orig["TIT2"].text)
        self.assertEqual(new["TPE1"].text, orig["TPE1"].text)
...
class TID3Write(TestCase):
    ...
    def test_wrong_encoding(self):
        t = ID3(self.filename)
        t.add(TIT2(encoding=Encoding.LATIN1, text=["\u0243"]))
    ...
    def test_1bfile(self):
        ...
        with open(self.filename, "wb") as f:
            f.write(b"!")
        t.save(self.filename)
        self.assertTrue(os.path.getsize(self.filename) > 1)
        with open(self.filename, "rb") as h:
            self.assertEqual(h.read()[-1], b"!"[0])
    ...
    def test_addframe(self):
        f = ID3(self.filename)
        self.assertTrue("TIT3" not in f)
        f["TIT3"] = TIT3(encoding=0, text="A subtitle!")
        f.save()
        id3 = ID3(self.filename)
        self.assertEqual(id3["TIT3"], "A subtitle!")

    def test_changeframe(self):
        f = ID3(self.filename)
        self.assertEqual(f["TIT2"], "Silence")
        f["TIT2"].text = ["The sound of silence."]
        f.save()
        id3 = ID3(self.filename)
        self.assertEqual(id3["TIT2"], "The sound of silence.")

    def test_replaceframe(self):
        f = ID3(self.filename)
        self.assertEqual(f["TPE1"], ['piman', 'jzig'])
        f["TPE1"] = TPE1(encoding=0, text="jzig\x00piman")
        f.save()
        id3 = ID3(self.filename)
        self.assertEqual(id3["TPE1"], ["jzig", "piman"])

    def test_compressibly_large(self):
        f = ID3(self.filename)
        self.assertTrue("TPE2" not in f)
        f["TPE2"] = TPE2(encoding=0, text="Ab" * 1025)
        f.save()
        id3 = ID3(self.filename)
        self.assertEqual(id3["TPE2"], "Ab" * 1025)
    ...
    def test_nofile_silencetag(self):
        ...
        with open(self.filename, 'rb') as h:
            self.assertEqual(b'ID3', h.read(3))
        self.test_same()
    ...
    def test_empty_plustag_minustag_empty(self):
        ...
        id3.save()
        id3.delete()
        self.assertFalse(id3)
        with open(self.filename, 'rb') as h:
            self.assertEqual(h.read(10), b'')
    ...
    def test_delete_invalid_zero(self):
        ...
        with open(self.filename, 'rb') as f:
            self.assertEqual(f.read(10), b'abc')
    ...
    def test_frame_order(self):
        ...
        with open(self.filename, 'rb') as h:
            data = h.read()
        self.assertTrue(data.find(b"TIT2") < data.find(b"APIC"))
        self.assertTrue(data.find(b"TIT2") < data.find(b"COMM"))
        self.assertTrue(data.find(b"TALB") < data.find(b"APIC"))
        self.assertTrue(data.find(b"TALB") < data.find(b"COMM"))
        self.assertTrue(data.find(b"TIT2") < data.find(b"TALB"))

    def test_apic_last(self):
        # https://github.com/quodlibet/mutagen/issues/278
        f = ID3(self.filename)
        f.add(TYER(text=["2016"]))
...
class WriteForEyeD3(TestCase):
    ...
    def test_same(self):
        ...
        self.assertEqual(id3.frames["TALB"][0].text, "Quod Libet Test Data")
        self.assertEqual(id3.frames["TCON"][0].text, "Silence")
        self.assertEqual(id3.frames["TIT2"][0].text, "Silence")
        self.assertEqual(len(id3.frames["TPE1"]), 1)
        self.assertEqual(id3.frames["TPE1"][0].text, "piman/jzig")
    ...
    def test_addframe(self):
        ...
        id3 = eyeD3.tag.Tag(eyeD3.ID3_V2_4)
        id3.link(self.newsilence)
        self.assertEqual(id3.frames["TIT3"][0].text, "A subtitle!")
    ...
    def test_changeframe(self):
        ...
        id3 = eyeD3.tag.Tag(eyeD3.ID3_V2_4)
        id3.link(self.newsilence)
        self.assertEqual(id3.frames["TIT2"][0].text, "The sound of silence.")
...
class BadPOPM(TestCase):
    ...
    def test_read_popm_long_counter(self):
        f = ID3(self.filename)
        self.assertTrue("POPM:Windows Media Player 9 Series" in f)
        popm = f["POPM:Windows Media Player 9 Series"]
        self.assertEqual(popm.rating, 255)
        self.assertEqual(popm.count, 2709193061)
    ...
    def test_write_popm_long_counter(self):
        ...
        f.save()
        f = ID3(self.filename)
        self.assertTrue("POPM:foo@example.com" in f)
        self.assertTrue("POPM:Windows Media Player 9 Series" in f)
        popm = f["POPM:foo@example.com"]
        self.assertEqual(popm.rating, 125)
        self.assertEqual(popm.count, 2 ** 32 + 1)
...
class Issue69_BadV1Year(TestCase):
    ...
    def test_missing_year(self):
        ...
        self.assertEqual(tag["TIT2"], "hello world")
    ...
    def test_short_year(self):
        ...
        tag = ParseID3v1(data)
        self.assertEqual(tag["TIT2"], "hello world")
        self.assertEqual(tag["TDRC"], "0001")
    ...
    def test_long(self):
        ...
        self.assertEqual(len(s), 128)
        tag = ParseID3v1(s)
        self.assertEqual(tag["TDRC"], "1234")
