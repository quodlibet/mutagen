...
class TMP3(TestCase):
    ...
    def test_notmp3(self):
        self.failUnlessRaises(MP3Error, MP3, os.path.join(DATA_DIR, 'empty.ofr'))

        self.failUnlessRaises(MP3Error, MP3, os.path.join(DATA_DIR, 'emptyfile.mp3'))

    def test_too_short(self):
        self.failUnlessRaises(MP3Error, MP3, os.path.join(DATA_DIR, 'too-short.mp3'))
...
class TXingHeader(TestCase):
    def test_valid_info_header(self):
        data = (
            b'Info\x00\x00\x00\x0f\x00\x00:>\x00\xed\xbd8\x00\x03\x05\x07'
...
class TVBRIHeader(TestCase):
    def test_valid(self):
        ...
        data = (
            b'VBRI\x00\x01\t1\x00d\x00\x0c\xb05\x00\x00\x049\x00\x87\x00'
...
class TLAMEHeader(TestCase):
    ...
    def test_settings(self):
        ...
        assert s(3, 94, vbr_method=2, preset_used=0, bitrate=255) == '--abr 255+'
        ...
        assert s(3, 93, vbr_method=3, preset_used=1004) == '--preset fast standard'
        assert s(3, 93, vbr_method=3, preset_used=1005) == '--preset fast extreme'
        assert s(3, 93, vbr_method=3, preset_used=1006) == '--preset medium'
        assert s(3, 93, vbr_method=3, preset_used=1007) == '--preset fast medium'
