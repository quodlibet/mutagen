...
class TMid3v2(_TTools):
    ...
    def test_wcom_multiple(self):
        res, out = self.call('--WCOM', 'foo', '--WCOM', 'bar', self.filename)
        self.failUnlessEqual((res, out), (0, ''))
        f = ID3(self.filename)
        frames = f.getall('WCOM')
        assert len(frames) == 1
    ...
    def test_wxxx_escape(self):
        res, out = self.call('-e', '--WXXX', 'http\\://example.com/', self.filename)
        self.failUnlessEqual((res, out), (0, ''))
        f = ID3(self.filename)
        frames = f.getall('WXXX')
    ...
    def test_wxxx_multiple(self):
        res, out = self.call('--WXXX', 'A:B', '--WXXX', 'C:D', self.filename)
        self.failUnlessEqual((res, out), (0, ''))
        f = ID3(self.filename)
    ...
    def test_ufid(self):
        res, out, err = self.call2('--UFID', 'foo:bar', self.filename)
    ...
    def test_apic(self):
        ...
        res, out, err = self.call2(
            '--APIC', image_path + ':fooAPIC:3:image/jpeg', self.filename
        )
