...
def copy(src: str, dst: str, merge: bool, write_v1: bool = True,
         excluded_tags: Sequence[str] | None = None, verbose: bool = False) -> int:
...
def main(argv: Sequence[str]) -> int:
    ...
    _ = parser.add_option("-v", "--verbose", action="store_true", dest="verbose",
                      help="print out saved tags", default=False)
    _ = parser.add_option("--write-v1", action="store_true", dest="write_v1",
                      default=False, help="write id3v1 tags")
    _ = parser.add_option("-x", "--exclude-tag", metavar="TAG", action="append",
                      dest="x", help="exclude the specified tag", default=[])
    _ = parser.add_option("--merge", action="store_true",
                      help="Copy over frames instead of the whole ID3 tag",
                      default=False)
    (options, args) = parser.parse_args(list(argv[1:]))

    options.x = cast(list[str], options.x)
    options.verbose = bool(options.verbose)
    options.write_v1 = bool(options.write_v1)
    options.merge = bool(options.merge)
